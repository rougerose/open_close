...
class StoreHours
{
	 ...
	 * @param array $config
	 */
	public function __construct($hours = [], $exceptions = [], $config = []) {
	...
	public function hours_this_week($groupSameDays = false) {
		...
		return ($groupSameDays === true) ? $this->hours_this_week_grouped() : $this->hours_this_week_simple();
	...
	private function format_hours(array $ranges) {
		...
		foreach ($ranges as $range) {
			...
			$hoursparts[] = date($this->config['format'], $start) . $this->config['separator'] . date(
				$this->config['format'],
				$end
			);
		}
